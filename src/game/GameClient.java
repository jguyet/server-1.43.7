...
public class GameClient {
    ...
    public long timeLastTradeMsg = 0, timeLastRecrutmentMsg = 0, timeLastAlignMsg = 0, timeLastChatMsg = 0, timeLastIncarnamMsg = 0, timeLastTaverne, lastPacketTime = 0, action = 0, depla = 0, readyDup = 0;;
        ...
        switch (packet.charAt(1)) {
            case 'a':
                ...
                if(Config.INSTANCE.getAZURIOM()) {
                    ogrineAcc3 = player.getAccount().getWebAccount().getPoints();
                }
                else{
                    ogrineAcc3 = player.getAccount().getOldpoints();
                }
            ...
            case 'C':
                ...
                int ogrineAcc1 = 0;
                if(Config.INSTANCE.getAZURIOM()) {
                    ogrineAcc1 = player.getAccount().getWebAccount().getPoints();
                }
                else{
                    ogrineAcc1 = player.getAccount().getOldpoints();
                }
            ...
            case 'l' :
            {
                if(player != null)
                {
                    ...
                    int precioBase = Config.INSTANCE.getPRIX_CHANGEMENT_CLASSE();
                    if(Config.INSTANCE.getAZURIOM()) {
                        SocketManager.send(player, "bOC" + player.getAccount().getWebAccount().getPoints() + "^" + precioBase);
                    }
                    else{
                        SocketManager.send(player, "bOC" + player.getAccount().getOldpoints() + "^" + precioBase);
                    }
                }
            }
            break;
            case 'L' :
            {
                ...
                int ogrineAcc = 0;
                if(Config.INSTANCE.getAZURIOM()) {
                    ogrineAcc = player.getAccount().getWebAccount().getPoints();
                }
                else{
                    ogrineAcc = player.getAccount().getOldpoints();
                }
                ...
                if(difference < 0)
                {
                    ...
                    if(Config.INSTANCE.getAZURIOM()) {
                        player.getAccount().getWebAccount().setPoints(difference);
                    }
                    else{
                        player.getAccount().setOldpoints(difference);
                    }
            ...
            case 'n':
                ...
                int ogrineAcc2 = 0;
                if(Config.INSTANCE.getAZURIOM()) {
                    ogrineAcc2 = player.getAccount().getWebAccount().getPoints();
                }
                else{
                    ogrineAcc2 = player.getAccount().getOldpoints();
                }
    ...
    private void createTonique(String packet) {
        ...
        this.player.LastTonicProposed[0] = -1;
        this.player.LastTonicProposed[1] = -1;
        this.player.LastTonicProposed[2] = -1;
        this.player.lastTonicPacket = "";
    ...
    private void LinkItem(String packet, Player player)
    {
        ...
        if(!object.isAttach()){
            ...
            SocketManager.GAME_SEND_MESSAGE(player, "Votre objet a été lié!",Constant.COULEUR_INFO);
        }
        else{
            object.dettachToPlayer(player);
            SocketManager.GAME_SEND_MESSAGE(player, "Votre objet a été délié!",Constant.COULEUR_INFO);
        }
    ...
    private void MorphItemDiscard(String packet, Player player)
    {
        ...
        if(object.getTxtStat().containsKey(Constant.APPARAT_ITEM)) {
            ...
            object.getTxtStat().remove(Constant.APPARAT_NAME);
            object.getTxtStat().remove(Constant.APPARAT_NAME2);
            object.getTxtStat().remove(Constant.STATS_OWNER_1);
        ...
        // MIMIBIOTE A RESTORE
        GameObject object1 = World.world.getObjTemplate(4).createNewItem(object.getQuantity(), false,0);
        if (player.addObjet(object1, true)) {
            World.world.addGameObject(object1, true);
        }
        object1.setModification();

        // OBJETS DEMIMIBIOTE
        if(object.getMimibiote() != -1) {object.setMimibiote(-1);}
        object.setModification();
        Database.getStatics().getObjectData().update(object);

        SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, object);
        ...
        SocketManager.GAME_SEND_MESSAGE(player, "Votre objet à bien retrouvé son apparence !",Constant.COULEUR_INFO);
    }
    ...
    private void MorphitemChange(String packet, Player player) {
        ...
        if (!verif.contains("|")){
            SocketManager.GAME_SEND_MESSAGE(player, "Une erreur c'est produite !",Constant.COULEUR_ECHEC);
            return;
        }
        ...
        GameObject objToChange = player.getItems().get(objectID);
        if (objToChange == null){
            SocketManager.GAME_SEND_MESSAGE(player, "Une erreur c'est produite !",Constant.COULEUR_ECHEC);
            return;
        }
        if (objToChange.getTxtStat().containsKey(Constant.APPARAT_ITEM)){
            SocketManager.GAME_SEND_MESSAGE(player, "Vous ne pouvez encore changer cette item d'apparance !",Constant.COULEUR_ECHEC);
            return;
        }
        if (objToChange.getStats().getEffects().containsKey(Constant.OBVIJEVANT_APPARENCE))
        {
            SocketManager.GAME_SEND_MESSAGE(player, "Vous ne pouvez pas mimibioter un item qui a un objet vivant !",Constant.COULEUR_ECHEC);
        }

        ObjectTemplate T = player.getItems().get(ApparatID).getTemplate();
        if (T == null || objToChange.getTemplate().getType() != T.getType() || !Constant.MimibioteItem(T.getType()) || !Constant.MimibioteItem(objToChange.getTemplate().getType())){
            SocketManager.GAME_SEND_MESSAGE(player, "Les deux objet ne peuvent êtres lié !",Constant.COULEUR_ECHEC);
            return;
        }
        ...
        if(obj.getTemplate().getType() == Constant.ITEM_TYPE_FAMILIER){
            pet = World.world.getPetsEntry(objectID);
            if(pet != null) {
                World.world.removePetsEntry(objectID);
                Database.getStatics().getPetData().delete(objectID);
            }
            else{
                SocketManager.GAME_SEND_MESSAGE(player, "Votre familié n'est pas valide car il a déja été utilisé en mimibiote",Constant.COULEUR_ECHEC);
                return;
            }
            //World.world.addPetsEntry(new PetEntry(id, getId(), pet.getLastEatDate(), 0, 10, 0, false));
        ...
        if(mimi != null)
        {
            // On delete un des mimibiotes
            player.removeItem(mimi.getGuid(), 1, true, true);
        }
        else{
            SocketManager.GAME_SEND_MESSAGE(player, "Aucun Miminote n'a été trouvé sur ton perso",Constant.COULEUR_ECHEC);
            return;
        }

        // On delete un des items d'apparat utilisé
        player.removeItem(ApparatID,1,true,true);

        // On delete l'item utilisé sur l
        ...
        int rarity = objToChange.getRarity();
        // On crée un nouvel item en ajoutant les infos d'apparat
        GameObject newObj = new GameObject(-1, objToChange.getTemplate().getId(), 1, Constant.ITEM_POS_NO_EQUIPED, stats, effetArme, new HashMap<Integer, Integer>(), effetText, 0, rarity,T.getId());
        // obj.attachToPlayer(player);
        if (newObj != null) {
            //target.sendMessage("Nouvel Item "+newObj.getTemplate().getName() + " " + newObj.getRarity() + " " + newObj.getQuantity() );
            if (player.addObjet(newObj, true)){
                World.world.addGameObject(newObj, true);
                if(newObj.getTemplate().getType() == Constant.ITEM_TYPE_FAMILIER) {
                    World.world.addPetsEntry(new PetEntry(newObj.getGuid(), pet.getTemplate(), pet.getLastEatDate(), pet.getQuaEat(), pet.getPdv(), pet.getCorpulence(), pet.getIsEupeoh()));
                    Database.getStatics().getPetData().add(newObj.getGuid(), pet.getLastEatDate(), pet.getTemplate());
                }
            }

        }

        //SocketManager.GAME_SEND_ASK(this, this.player);
        //SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, object);
        SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(this.player, obj);
        SocketManager.GAME_SEND_OCK_ACTUALISED_OBJECT(this.player, obj);
        SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(this.player, newObj);
        SocketManager.GAME_SEND_OCK_ACTUALISED_OBJECT(this.player, newObj);
        SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this.player.getCurMap(), this.player.getId());
        SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(this.player.getCurMap(), this.player);

        SocketManager.GAME_SEND_Ow_PACKET(player);
        SocketManager.GAME_SEND_MESSAGE(player, "Votre objet à bien été transformer !",Constant.COULEUR_INFO);
    }
            ...
            if (yo.getConditions() == null || yo.getConditions().length() == 0) {
                ...
                if (yo.getConditions().equals("ISVIP") && perso.getAccount().getVip() == 1) {
                    ...
                    if(jobs.size() >= 6){
                        ...
                        if(i>=6){
                            newTitreifKamas(perso, yo);
                            perso.sendMessage("Tu possèdes dorénavant le titre "+ yo.getName() );
                        }
                        else{
                ...
                else if (yo.getConditions().equals("DON1")){
                    int role = perso.getAccount().getWebAccount().getRole();
                    if(role >= 4){
                        newTitreifKamas(perso, yo);
                        perso.sendMessage("Tu possèdes dorénavant le titre "+ yo.getName() );
                    }
                    else{
                        perso.sendMessage("Tu n'as pas fait de dons pour le serveur ou celui n'a pas encore été traité par le staff");
                    }

                }
                else if (yo.getConditions().equals("DON2")){
                    Database.getSites().getAccountWebData().load(perso.getAccount().getId());
                    int role = perso.getAccount().getWebAccount().getRole();
                    if(role == 5){
                        newTitreifKamas(perso, yo);
                        perso.sendMessage("Tu possèdes dorénavant le titre "+ yo.getName() );
    ...
    public void UnsetItem(GameObject exObj){
    ...
    private void VerifToSet(Map<Integer, Long> itemtoEquip,List<GameObject> itemsEquipe, int pos){
        ...
        if (itemtoEquip.get(pos) != null) {
            ...
            if (player.getItems().get(itemtoEquip.get(pos)) != null) {
                if (Objold != null) {
                    if (ToEquipGUID != Objold.getGuid() || pos == Constant.ITEM_POS_ARME) {
                        if (pos == Constant.ITEM_POS_ARME) {
                            GameObject bouclier = player.getObjetByPos3(Constant.ITEM_POS_BOUCLIER, itemsEquipe);
                            if (bouclier != null) {
                                UnsetItem(bouclier);
                            }
                        }
                        UnsetItem(Objold);
                        movementObject("OM" + itemtoEquip.get(pos) + "|" + pos);
                    }
                }
                else{
                    if (pos == Constant.ITEM_POS_ARME) {
                ...
                GameObject obj = World.world.getGameObject(itemtoEquip.get(pos));
                if (obj != null) {
                    String Name = obj.getTemplate().getName();
                    this.player.sendMessage("[WARNING] Votre équipement " + Constant.GetNameByPos(pos) + " : '" + Name + "' n'a pas été trouvé");
                } else {
                    this.player.sendMessage("[ERROR] Votre équipement '" + itemtoEquip.get(pos) + "' n'a pas été trouvé dans le monde");
                }
            }
        }
        else{
    ...
    private void setSets(String packet){
        try {
            if(Constant.isInGladiatorDonjon(player.getCurMap().getId()) || player.getCurMap().getId() == 12277 ){
                this.player.sendMessage("Impossible de modifier son équipement au Gladiatrool");
                return;
            }
            ...
            if(set != null){
                ...
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_AMULETTE);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_ANNEAU1);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_ANNEAU2);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_CEINTURE);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_BOTTES);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_COIFFE);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_CAPE);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_ARME);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_FAMILIER);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DRAGODINDE);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DOFUS1);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DOFUS2);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DOFUS3);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DOFUS4);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DOFUS5);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_DOFUS6);
                VerifToSet(itemtoEquip,itemsEquipe,Constant.ITEM_POS_BOUCLIER);

                this.player.sendMessage("Votre équipement rapide '" + set.getName() + "' a été équipé");
            }
            else{
                this.player.sendMessage("[ERROR] Votre équipement rapide n°" + nb + " n'a pas été trouvé");
            }
        }
        catch (Exception e){
            e.printStackTrace();
            System.out.println("ERROR dans le chargement du set : " + e);

        }
    }

    private void createSet(String packet){
        try {
            String[] packetSplit = packet.split("\\|");
            int nb = Integer.parseInt(packetSplit[0]);
            String name = packetSplit[1];
            int icon = Integer.parseInt(packetSplit[2]);
            ...
            if( World.world.getSetByPersoIDandNB(playerId,nb) ==null ) {
                ...
                this.player.sendMessage("Vous avez modifié l'équipement rapide : " + name);
            }
        }
        catch (Exception e){
            this.player.sendMessage("[ERREUR] Une erreur est survenue lors de la création de l'équipement rapide");
    ...
    private void deleteSet(String packet){
        int nb = Integer.parseInt(packet);
        int playerId = player.getId();
        ...
        if(newset != null) {
            if (!Database.getStatics().getSetsData().delete(newset)) {
                this.player.sendMessage("[ERREUR] Une erreur est survenue lors de la suppression de l'équipement rapide");
                return;
            }
            this.player.sendMessage("Vous avez supprimé l'équipement rapide : " + Name);
        }
        else{
    ...
    private void AegnorService(String packet) {
        switch (packet.charAt(2))
        {
            case 'N':
                ...
                break;
            case 'C': // Création d'équipement rapide
                createSet(packet.substring(3));
                break;
            case 'U': // Utilisation d'équipement rapide
                setSets(packet.substring(3));
                break;
            case 'B': // Suppression d'équipement rapide
                deleteSet(packet.substring(3));
                break;
        }
    }
    ...
    private void showMonsterInfo(String Idmonstre) {
        try {
            ...
            {
                if(!(drop.getisOnAllMob())) {

                    double prospecting = this.player.getProspection() / 100.0;
                    if (prospecting < 1) prospecting = 1;
                    Double chanceSolo = 0.0;
                    if(drop.isNoPPInfluence()) {
                        chanceSolo = drop.getPercentbyGrade(1) * Config.INSTANCE.getRATE_DROP() * chancebasesolo * 2;
                    }
                    else{
                        chanceSolo = drop.getPercentbyGrade(1) * prospecting * Config.INSTANCE.getRATE_DROP() * chancebasesolo * 2;
                    }
                    ...
                    switch (drop.getAction()) {
                        ...
                        case 1:// Is meat so..
                            seuils = "Chasseur niveau" + level;
                            break;

                        case 2:// Verification of the condition (MAP)
                            seuils = "Map spécifique";
                            break;
                        ...
                        case 4: // Quete
                            seuils = "Avoir la quete "+drop.getCondition();
                            break;
                        ...
                        case 6: // Avoir l'objet
                            seuils = "Avoir l'objet "+World.world.getObjTemplate(Integer.parseInt(drop.getCondition())).getName();
                            break;

                        case 7:// Verification of the condition (MAP) mais pas plusieurs fois
                            seuils = "Map spécifique, 1 fois";
                            break;
                    ...
                    if( ArrayUtils.contains( Constant.FILTER_RESSOURCES, objectTemplate.getType() ) && (objectTemplate.getType() != Constant.ITEM_TYPE_CLEFS) && (objectTemplate.getId() != 12885) ) {
                        quantite = "1 à " + Config.INSTANCE.getRATE_DROP() +" + diff par monstre";
                    }
                    else{
                        quantite = "1 par monstre";
                    }
                    drops += itemId + "," + seuils + "#" + percent + "#" + quantite + ";";

                }
                else{
                    //System.out.println("Etrange" + drop.getObjectId());
                }
            }
            if(drops.length()>0){
            ...
            for (Map.Entry<Integer, Monster.MobGrade> entry : grades.entrySet()) {
                ...
                Map<Integer, SpellGrade> Spells = actualgrade.getSpells();
                String SpellString = "";
                for (Map.Entry<Integer, SpellGrade> spell : Spells.entrySet()) {
                    SpellGrade sort =  spell.getValue();
            ...
            case 'p' : //reset stats and nerrf spell
                switch (packet.charAt(2)) {
                    case 'c' :
                        if(!player.isMorph()) {
                            if(player.getFight() == null) {
                                player.Restat_Stats(true);
                                SocketManager.GAME_SEND_MESSAGE(player, "Vos caractéristiques ont été réinitialisées.", "2997F1");
                            }
                            else{
                                SocketManager.GAME_SEND_MESSAGE(player, "Tu ne peux pas restat en combat", "B9121B");
                            }
                        }
                        else{
                            SocketManager.GAME_SEND_MESSAGE(player, "Tu ne peux pas restat dans l'état actuel", "B9121B");
                        }
                        break;
                    case 's' :
    ...
    private void buy(String packet) {
            ...
            if (seller != null && seller != this.player) {
                ...
                if (qua == itemStore.getQuantity()) {
                    ...
                    if (this.player.addObjet(clone, true))
                        World.world.addGameObject(clone, true);
                ...
                Logging.getInstance().write("Object", "BuyModeMarchand : "
                        + this.player.getName() + " : achat de "
                        + itemStore.getTemplate().getName() + " x"
                        + qua + " venant du compte "
                        + seller.getName());
        ...
        else if( exchangeAction.getType() == ExchangeAction.TRADING_WITH_BOUTIQUE)
        {
            try {
                ...
                if ( (template.getType() == Constant.ITEM_TYPE_FAMILIER || template.getType() == Constant.ITEM_TYPE_CERTIF_MONTURE ) && qua > 1) {
                ...
                if (template.getPoints() > 0 && template.getBoutique() > 0) {
                    ...
                    int value = 0;
                    if(Config.INSTANCE.getAZURIOM()) {
                        value = template.getPoints() * qua;
                        points = this.account.getWebAccount().getPoints();
                    }
                    else {
                        value = template.getPoints() * qua;
                        points = this.account.getOldpoints();
                    }
                    ...
                    if(Config.INSTANCE.getAZURIOM()) {
                        this.account.getWebAccount().setPoints(points - value);
                    }
                    else{
                        this.account.setOldpoints(points - value);
                    }
        ...
        else
        {
            ...
            try {
                ...
                if (template.getPoints() > 0 && (npcTemplate.getInformations() & 0x4) == 4) {
                    if(this.account.getWebAccount() == null){
                        String mess = "Tu ne peux pas charger tes points boutique car tu n'as pas affilié ton compte à un compte web.";
                        this.player.sendMessage(mess);
                        return;
                    }
                    int value = template.getPoints() * qua, points = this.account.getWebAccount().getPoints();
    ...
    private void bigStore(String packet) {
        ...
        switch (packet.charAt(2)) {
            ...
            case 'T'://Demande des template de la cat?gorie
                int categ =1;
                try {
                    categ = Integer.parseInt(packet.substring(3));
                }
                catch (NumberFormatException e) {
                    // Handle the error appropriately
                    World.world.logger.error("Invalid input for bigStore: " + packet, e);
                    // You can also add some fallback logic if needed
                }
                int hdvid = 0;
    ...
    private void ready() {
        ...
        if (exchangeAction.getType() == ExchangeAction.TRADING_WITH_NPC_SELL_STUFF && value instanceof PlayerExchange.NpcExchangeSellItem) {
            ((PlayerExchange.NpcExchangeSellItem) value).toogleOK(false);
            if(this.player.getExchangeAction() != null) {
                if(((PlayerExchange.NpcExchangeSellItem) this.player.getExchangeAction().getValue()).getKamas(true) != 0) {
                    ((PlayerExchange.NpcExchangeSellItem) value).toogleOK(true);
                }
            }
        }

        if (exchangeAction.getType() == ExchangeAction.TRADING_WITH_NPC_BREAK && value instanceof PlayerExchange.NpcBreakItem) {
            ((PlayerExchange.NpcBreakItem) value).toogleOK(false);
            if (((PlayerExchange.NpcBreakItem) value).getObjects().isEmpty())
                return;
            ...
            ((PlayerExchange.NpcBreakItem) value).toogleOK(true);
        }
        ...
        switch(this.player.getExchangeAction().getType()) {
                    ...
                    case 'O'://Objet ?
                        if (packet.charAt(3) == '+') {
                            ...
                            try {
                                ...
                                ((PlayerExchange.NpcExchangeSellItem) this.player.getExchangeAction().getValue()).setKamas(true, kamas);
                                ((PlayerExchange.NpcExchangeSellItem) this.player.getExchangeAction().getValue()).addItem(guid, qua);
                            } catch (NumberFormatException e) {
                                World.world.logger.error("Error Echange NPC '" + packet + "' => " + e.getMessage());
                                e.printStackTrace();
                                return;
                            ...
                            try {
                                ...
                                if (obj == null)
                                    return;
                                if (qua > ((PlayerExchange.NpcExchangeSellItem) this.player.getExchangeAction().getValue()).getQuaItem(guid, false))
                                    return;
                                ...
                                ((PlayerExchange.NpcExchangeSellItem) this.player.getExchangeAction().getValue()).setKamas(true, kamas);
                                ((PlayerExchange.NpcExchangeSellItem) this.player.getExchangeAction().getValue()).removeItem(guid, qua);
                            } catch (NumberFormatException e) {
                                World.world.logger.error("Error Echange NPC '" + packet + "' => " + e.getMessage());
                                e.printStackTrace();
                                return;
                ...
                switch (packet.charAt(2)) {
                    ...
                    case 'G'://Kamas
                        try {
                            ...
                            if (this.player.getKamas() < numb)
                                numb = this.player.getKamas();
                            ((PlayerExchange.NpcBreakItem) this.player.getExchangeAction().getValue()).setKamas(false, numb);
                        } catch (NumberFormatException e) {
                            World.world.logger.error("Error Echange kamas '" + packet + "' => " + e.getMessage());
                            e.printStackTrace();
                            return;
            ...
            case ExchangeAction.CRAFTING:
                ...
                if (packet.charAt(2) == 'O') //((JobAction) this.player.getExchangeAction().getValue()).getJobCraft() == null
                {
                    ...
                    for(String part : packet.split(";")) {
                        try {
                            ...
                            if (c == '+') {
                                ...
                                if(obj.getObvijevanId() != 0){
                                    this.player.sendMessage("Il n'est pas possible de craft/FM un item Obvijevant, commence par dissocier ton item de son Obvejivant");
                                    return;
                                }

                                if(obj.getMimibiote() != -1){
                                    this.player.sendMessage("Il n'est pas possible de craft/FM un item Mimibiote, commence par dissocier ton item de son mimibiote");
                                    return;
                                }
                    ...
                    if (((JobAction) this.player.getExchangeAction().getValue()).getJobCraft() == null) {
                        JobAction job = ((JobAction) this.player.getExchangeAction().getValue());
   ...
   /* private void recursiveBreakingObject(BreakingObject breakingObject, final int i, int count) {
    ...
    private void request(String packet) {
        ...
        else if (packet.substring(2, 4).equals("19") && this.player.getExchangeAction() == null) {// To Selling stuff
            int id = Integer.parseInt(packet.substring(5));

            if (this.player.getCurMap().getNpc(id) != null) {
                PlayerExchange.NpcExchangeSellItem ech = new PlayerExchange.NpcExchangeSellItem(this.player, this.player.getCurMap().getNpc(id).getTemplate());
                ExchangeAction<PlayerExchange.NpcExchangeSellItem> exchangeAction = new ExchangeAction<>(ExchangeAction.TRADING_WITH_NPC_SELL_STUFF, ech);
                this.player.setExchangeAction(exchangeAction);
                SocketManager.GAME_SEND_ECK_PACKET(this.player, 2, String.valueOf(id));
            }
        }
        else if (packet.substring(2, 4).equals("20") && this.player.getExchangeAction() == null) { // To break object
            int id = Integer.parseInt(packet.substring(5));

            if (this.player.getCurMap().getNpc(id) != null) {
                PlayerExchange.NpcBreakItem ech = new PlayerExchange.NpcBreakItem(this.player, this.player.getCurMap().getNpc(id).getTemplate());
                ExchangeAction<PlayerExchange.NpcBreakItem> exchangeAction = new ExchangeAction<>(ExchangeAction.TRADING_WITH_NPC_BREAK, ech);
                this.player.setExchangeAction(exchangeAction);
                SocketManager.GAME_SEND_ECK_PACKET(this.player, 2, String.valueOf(id));
            }
        }
        ...
        switch (packet.charAt(2)) {
            case '0'://Si NPC
                ...
                if (npc != null) {
                    ...
                    int itemIDMoney = npc.getTemplate().UsespecificMoney();
                    if(npc.getTemplate().UsespecificMoney() != -1){
                        this.player.sendMessage("Attention, ce PNJ achète avec des "+ World.world.getObjTemplate(itemIDMoney).getName());
                    }
    ...
    public static void leaveExchange(Player player) {
        ...
        switch(exchangeAction.getType()) {
            ...
            case ExchangeAction.TRADING_WITH_NPC_EXCHANGE:
                ...
                break;
            case ExchangeAction.TRADING_WITH_NPC_SELL_STUFF:
                ((PlayerExchange.NpcExchangeSellItem) exchangeAction.getValue()).cancel();
                break;
            case ExchangeAction.TRADING_WITH_NPC_BREAK:
                ((PlayerExchange.NpcBreakItem) exchangeAction.getValue()).cancel();
                break;
            case ExchangeAction.CRAFTING_SECURE_WITH:
                if(exchangeAction.getValue() instanceof Integer) {
                    Player target = World.world.getPlayer((Integer) exchangeAction.getValue());
    ...
    private void gameParseDeplacementPacket(GameAction GA) {
        ...
        if (this.player.getFight() == null) {
            ...
            GA.args = path;
            this.depla = System.currentTimeMillis();
            ...
            if (this.player.walkFast) {
                ...
                if (targetCell.getObject() != null) {
                    logger.debug("Object Interactif {} sur la cell {} apres action",
                            targetCell.getObject().getId(), this.player.getCurMap().getId()+";"+targetCell.getId());
    ...
    private void gameTryCastSpell(String packet) {
        try {
            ...
            if(this.player.getCurrentCompagnon() == null) {
                if (fight != null) {
                    ...
                    if (SS != null){
                        ...
                        this.player.getFight().cast(this.player.getFight().getFighterByPerso(this.player), () -> this.player.getFight().tryCastSpell(this.player.getFight().getFighterByPerso(this.player), SS, cellId));
                        SocketManager.GAME_SEND_GA_PACKET(this.player.getGameClient(), "", "0", "", "");
                        this.player.sendMessage("Vous ne possédez pas le sort nécessaire");
                    }
                }
            }
            else {
                ...
                if (fight != null) {
                    ...
                    if (SS != null) {
                        ...
                        this.player.getFight().cast(this.player.getFight().getFighterByPerso(player), () -> this.player.getFight().tryCastSpell(this.player.getFight().getFighterByPerso(player), SS, cellId));
                        SocketManager.GAME_SEND_GA_PACKET(this.player.getGameClient(), "", "0", "", "");
                        this.player.sendMessage("Vous ne possédez pas le sort nécessaire (invo)");
                    }
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            if (Logging.USE_LOG)
                Logging.getInstance().write("Error", "onTrySpell error" + e.getMessage() + " | " + packet);
        }
    }
    ...
    private void gameTryCac(String packet) {
        try {
            ...
            if(this.player.getCurrentCompagnon() == null) {
                ...
                Player player = this.player.getCurrentCompagnon().getPlayer();
                if (this.player.getFight() != null && this.player.getFight().getCurAction().isEmpty())
                    this.player.getFight().cast(currentFighter, () -> this.player.getFight().tryCaC(player, cell));
            }
            ...
            e.printStackTrace();
            if (Logging.USE_LOG)
                Logging.getInstance().write("Error", "onTryCac error" + e.getMessage() + " | " + packet);
        }
    }
    ...
    private void leaveFight(String packet) {
        ...
        if (id > 0) {
            ...
            /*if(fight.getFighterByOrdreJeu() != fight.getFighterByPerso(this.player) ){ A REFAIRE SI LE MAITRE BUG ENCORE EN SORTIE DE COMBAT LORS ABANDON
                if(this.player.PlayerList1.isEmpty()){
                    if(this.player.controleinvo){
                        this.player.sendMessage("Tu ne peux pas abandonner le combat en dehors de ton tour de jeu si as le controle d'invocation activé ");
                    }
                    else{
                     ...
                     if(this.player.oneWindows){
                         this.player.sendMessage("Tu ne peux pas abandonner le combat en dehors de ton tour de jeu si tu es le maitre avec le OneWindows activé");
                     }
                     else{
                         if(this.player.controleinvo){
                             this.player.sendMessage("Tu ne peux pas abandonner le combat en dehors de ton tour de jeu si as le controle d'invocation activé ");
    ...
    private void readyFight(final String packet) {
        ...
        this.readyDup = System.currentTimeMillis();
            ...
            if ( this.player.getSlaves().size() > 0) {
                ...
                if (ok) {
                    for (Player slave :  this.player.PlayerList1) {
                        ...
                        if (slave.getAccount() != null) {
                            if (slave.getAccount().getGameClient() != null) {
                                //On dupliqueF la game action du maitre pour les slaves
                                // slave.getAccount().getGameClient().gameParseDeplacementPacket(GA);
                                try{
    ...
    private void boostSpellGuild(String packet) {
        ...
        if (G2.getSpells().containsKey(spellID)) {
            ...
            GameServer.a("LA guilde n'a pas le spell "+ spellID);
        }
    }
    ...
    private void changeMemberProfil(String packet) {
        ...
        int guid = Integer.parseInt(infos[0]); // ID du perso
        int rank = Integer.parseInt(infos[1]); // Rang de guilde a mettre
        byte xpGive = Byte.parseByte(infos[2]); // % d'xp
        int right = Integer.parseInt(infos[3]); // Les droits

        boolean canDo = false;
        Player p = World.world.getPlayer(guid); //Cherche le personnage a qui l'on change les droits dans la mémoire

        GuildMember lanceur = this.player.getGuildMember();

        //Récupération du personnage à changer, et verification de quelques conditions de base
        if (p == null) // Si le joueur n'existe pas on fait rien
            return;

        if (p.getGuild() == null) { // Si le joueur n'a pas de guilde
            SocketManager.GAME_SEND_gK_PACKET(this.player, "Ea");
            return;
        }
        GuildMember toChange = p.getGuildMember();
        ...
        if (lanceur.getRank() == 1) //Si c'est le meneur
        {
            if (rank == 1 && toChange != lanceur) //Si il met un autre membre "Meneur"
            {
                lanceur.setAllRights(2, (byte) -1, 29694, this.player); //Met le meneur "Bras droit"
            }
            canDo = true;
        }
        else  //Sinon, c'est un membre normal
        {
            if(toChange.getRank() == 1){
                canDo= false; // Le gars n'est pas meneur et veut mettre quelqu'un meneur ? Il est fou
            }
            else{
                // Alors la on va check ce qu'on change et si on peut le faire
                if(toChange.getRank() != rank && lanceur.canDo(Constant.G_RANK)) {
                    canDo = true;
                }

                // Alors la on va check ce qu'on change et si on peut le faire
                if(toChange.getRights() != right && lanceur.canDo(Constant.G_RIGHT)) {
                    canDo= true;
                }

                // Alors la on va check ce qu'on change et si on peut le faire
                if(toChange != lanceur) {
                    if (toChange.getXpGive() != xpGive && lanceur.canDo(Constant.G_ALLXP)) {
                        canDo = true;
                    }
                }
                else{
                    if (toChange.getXpGive() != xpGive && lanceur.canDo(Constant.G_HISXP)) {
                        canDo = true;
                    }
                }
            }
        ...
        if(canDo) {
            toChange.setAllRights(rank, xpGive, right, p);
            ...
            this.player.sendMessage("Vous n'avez pas les droits necessaire pour effectuer cette action");
            return;
        }
    }
    ...
    private void parseObjectPacket(String packet) {
        switch (packet.charAt(1)) {
            ...
            case 'm':
                ...
                break;
            case 'l':
                LinkItem(packet, this.player);
                break;
            case 'U':
                useObject(packet);
                break;
            case 'S':
                quickSendObject(packet);
                break;
            case 'x':
    ...
    private synchronized void movementObject(String packet) {
        if(Constant.isInGladiatorDonjon(player.getCurMap().getId()) || player.getCurMap().getId() == 12277){
            player.sendMessage("Impossible de modifier son équipement au Gladiatrool");
            return;
        }
        ...
        try {
            ...
            if (position == Constant.ITEM_POS_FAMILIER && object.getTemplate().getType() != Constant.ITEM_TYPE_FAMILIER && this.player.getObjetByPos(position) != null) {
                ...
                if (!object.getTemplate().getConditions().equalsIgnoreCase("") && !World.world.getConditionManager().validConditions(this.player, object.getTemplate().getConditions())) {
                    SocketManager.GAME_SEND_Im_PACKET(this.player, "119|44;"+ object.getTemplate().getName()); // si le this.player ne v?rifie pas les conditions diverses
                    return;
                }
                if ((position == Constant.ITEM_POS_BOUCLIER && this.player.getObjetByPos(Constant.ITEM_POS_ARME) != null) || (position == Constant.ITEM_POS_ARME && this.player.getObjetByPos(Constant.ITEM_POS_BOUCLIER) != null)) {
                    if (this.player.getObjetByPos(Constant.ITEM_POS_ARME) != null) {
                        if (this.player.getObjetByPos(Constant.ITEM_POS_ARME).getTemplate().isTwoHanded() && !(ArrayUtils.contains(Constant.SHIELD_HANDLING_EXCEPTIONS ,object.getTemplate().getId()) ) ) {
                            SocketManager.GAME_SEND_Im_PACKET(this.player, "119|44;"+ object.getTemplate().getName() ); // si le this.player ne v?rifie pas les conditions diverses
                            this.player.sendMessage("Vous ne pouvez pas équiper un bouclier lorsque vous avez une 'arme à deux mains' d'équipé");
                            return;
                        }
                    }
                    else {
                        if ( object.getTemplate().isTwoHanded() && !(ArrayUtils.contains(Constant.SHIELD_HANDLING_EXCEPTIONS ,this.player.getObjetByPos(Constant.ITEM_POS_BOUCLIER).getTemplate().getId()) )) {
                            SocketManager.GAME_SEND_Im_PACKET(this.player, "119|44;" + object.getTemplate().getName()); // si le this.player ne v?rifie pas les conditions diverses
                            this.player.sendMessage("Vous ne pouvez pas équiper une 'arme à deux mains' lorsque vous avez un bouclier d'équipé");
                            return;
                        }
                    }
                ...
                if (exObj != null)//S'il y avait d?ja un objet sur cette place on d?s?quipe
                {
                    ...
                    {
                        if (object.getPosition() > Constant.ITEM_POS_DRAGODINDE) {
    ...
    public void useFragment(int ItemGUID){
        String Packet = "OU"+ItemGUID+"";
        useObject(Packet);
    }

    private void quickSendObject(String packet) {
        if(Constant.isInGladiatorDonjon(player.getCurMap().getId()) || player.getCurMap().getId() == 12277){
            player.sendMessage("Impossible de transferer un objet au Gladiatrool");
            return;
        }
        ...
        try {
            ...
            try {
                targetGuid = Integer.parseInt(infos[2]);
            } catch (Exception e) {
                // ok
            }


            GameObject toSend = World.world.getGameObject(guid);
            Player receiver = World.world.getPlayer(targetGuid);
            if(!this.player.getParty().getPlayers().contains(receiver)){
                this.player.sendMessage(receiver.getName()+ " ne fait pas parti de votre groupe");
                return;
            }

            if(!player.getAccount().getCurrentIp().toString().equalsIgnoreCase(receiver.getAccount().getCurrentIp().toString())){
                this.player.sendMessage(receiver.getName()+ " n'est pas connecté sur la même IP");
                return;
            }

            if(toSend!=null && toSend.getQuantity() >= quantity && receiver != null && receiver.isOnline()) {
                if(toSend.getPosition() == -1)
                    this.sendObject(toSend,quantity,receiver);
                else
                    return;
            }
            else{
            ...
            SocketManager.GAME_SEND_Im_PACKET(player, "022;" + quantity + "~" + toSend.getTemplate().getId());

            if(receiver != null && receiver.isOnline())
                SocketManager.GAME_SEND_Im_PACKET(receiver, "021;" + quantity + "~" + toSend.getTemplate().getId());
    ...
    protected void sendObject(GameObject object, int Quantity , Player player) {
        if(object == null) return;
        if ((object.getQuantity() - Quantity) < 1) {
            this.player.removeItem(object.getGuid());
            int qty = object.getQuantity();
            SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(this.player, object.getGuid());
            if (!player.addObjet(object, true)) World.world.removeGameObject(object.getGuid());
        } else {
            object.setQuantity(object.getQuantity() - Quantity);
            SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(this.player, object);
            GameObject newObj = GameObject.getCloneObjet(object, Quantity);
            if (player.addObjet(newObj, true)) World.world.addGameObject(newObj, true);
        }
    }
    ...
    private void rideMount() {
        ...
        if (Constant.isInGladiatorDonjon(this.player.getCurMap().getId()) || this.player.getCurMap().getId() == 12277 ) {
            this.player.sendMessage("Impossible de modifier son équipement au Gladiatrool");
            return;
        }
    ...
    public void changeName(String packet) {
        ...
        if (name.length() > 20 || name.length() < 3 || name.contains("modo") || name.contains("admin") || name.contains("putain") || name.contains("administrateur") || name.contains("puta") || name.contains("aegnor") || name.contains("modera") )
            isValid = false;
        if (isValid) {
            int tiretCount = 0;
            char exLetterA = ' ';
    ...
    public void send(String packet) {
        try {
            ...
            if (Logging.USE_LOG)
                Logging.getInstance().write("Error", "Send fail : " + packet + " " + e.getMessage() );
            e.printStackTrace();
        }
    }
